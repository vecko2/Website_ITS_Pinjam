    ...
    <nav className="sticky top-0 z-40 border-b border-slate-200 bg-white">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
        ...
        <div className="flex items-center gap-4 text-sm">
          <Link href="/catalog" title="Katalog" className="text-slate-600 hover:text-[#1A3C6E]">
          ...
          {user ? (
            <>
              {user.role === "peminjam" && (
                <Link href="/cart" title="Keranjang" className="text-slate-600 hover:text-[#1A3C6E]">
                  <Image src={cartIcon} alt="Keranjang" width={22} height={22} className="object-contain" />
                </Link>
              )}
              {user.role === "pemilik" && (
                <Link href="/dashboard/owner/transactions" title="Transaksi" className="text-slate-600 hover:text-[#1A3C6E]">
                  <Image src={transaksiIcon} alt="Transaksi" width={22} height={22} className="object-contain" />
                </Link> 
              )}
              <NotificationBell />
              <Link href={user.role === "pemilik" ? "/dashboard/owner" : "/dashboard/borrower"} title="Dashboard"
